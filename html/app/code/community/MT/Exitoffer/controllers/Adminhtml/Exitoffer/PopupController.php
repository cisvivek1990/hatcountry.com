...
class MT_Exitoffer_Adminhtml_Exitoffer_PopupController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if ($id) {
            ...
            if ($model->getId()) {
                ...
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('exitoffer')->__('popup does not exist'));
                $this->_redirect('*/*/');
            }
        }
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('exitoffer')->__('Exit Intent Popup was successfully saved.'));
            ...
            try {
                if (is_numeric($request->getParam('field_id'))) {
                    ...
                    if ($popup->getContentType() == MT_Exitoffer_Model_Popup::CONTENT_TYPE_NEWSLETTER_SUBSCRIPTION_FORM) {
                        Mage::getModel('exitoffer/subscriber')->addDbColumn($request->getParam('name'), $request->getParam('type'));
                    }
                ...
                $result['error'] = $helper->__('An error occurred while saving field. Please review the log and try again.');
                Mage::logException($e);
            }
        }
    ...
    public function deleteFieldAjaxAction()
    {
        ...
        if (!$field->getId()) {
            ...
            try {
                ...
                $result['error'] = $helper->__('An error occurred while deleting field. Please review the log and try again.');
                Mage::logException($e);
            }
        }
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->getParam('id')) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('exitoffer')->__('Exit Intent Popup was successfully deleted.'));
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $this->_redirect('*/*/');
    ...
    public function fieldGridAction()
    {
        ...
        $this->getResponse()->setBody($this->getLayout()->createBlock('exitoffer/adminhtml_exitoffer_popup_edit_tabs_fields_grid')->toHtml());
    }
