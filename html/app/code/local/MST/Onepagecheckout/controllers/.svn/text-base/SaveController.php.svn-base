...
class MST_Onepagecheckout_SaveController extends Mage_Checkout_Controller_Action
{
    ...
    public function saveOrderAction()
    {
        ...
        try {
            ...
            if (!isset($result['redirect']) && !isset($result['error']))
            {
                ...
                if(!$ship_method)
                {
                    $result['error_messages'] = Mage::helper('onepagecheckout')->__('Please, select shipping method.');
                    $result['error']    = true;
                    $result['success']  = false;
                    ...
                    $this->getOnepagecheckout()->useShipping($ship_method);
                    $this->getOnepagecheckout()->getQuote()->collectTotals()->save();
                    $ship_methods = $this->getAvalaibleShippingMethods();
                    if(!in_array($ship_method, $ship_methods))
                    {
                        $result['error_messages'] = Mage::helper('onepagecheckout')->__('Please, check shipping address.');
                        $result['error']    = true;
                        $result['success']  = false;
                    }
                    else {
        ...
        if (isset($redirectUrl) && !$result['error']) {
            $result['redirect'] = $redirectUrl;
            $result['success'] = true;
        }
        if($result['success']){
    ...
    private function getAvalaibleShippingMethods()
    {
        $groups = Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getGroupedAllShippingRates();
        $addresss = $this->getOnepagecheckout()->getQuote()->getShippingAddress();

        $groups = Mage::getModel('hc_militaryshipping/shipping')
            ->restrictMilitaryShipping($groups, $addresss->getData('postcode'));

        $groups = Mage::getModel('hc_militaryshipping/shipping')
            ->restrictPOBox($addresss->getCountryId(), $addresss->getStreet(), $groups);
