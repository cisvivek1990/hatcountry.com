...
//        $html[] = '</a>';
    ...
    public function drawAllBrands()
    {
        $active = '';
        $url = 'all-brands.html';
        if (Mage::getSingleton('cms/page')->getIdentifier() == $url && Mage::app()->getFrontController()->getRequest()->getRouteName() == 'cms') {
            $active = ' act';
        }

        $id = "_brands";
        $html[] = '<div id="pt_menu' . $id . '" class="pt_menu pt_menu_no_child' . $active . '">';
        $html[] = '<div class="parentMenu">';
        $html[] = '<a href="/' . $url . '">';
        $html[] = '<span>Brands</span>';
        $html[] = '</a>';
        $html[] = '</div>';
        $html[] = '</div>';
        $html = implode("\n", $html);
