...
class HC_Urlrewrite_Model_Url extends Mage_Catalog_Model_Url
{
    /**
    ...
    {
       $visibility = Mage::getModel('catalog/product')
           ->getResource()
           ->getAttributeRawValue($product->getId(), 'visibility', Mage::app()->getStore());

        if ($category->getId() == $category->getPath()
            || (int)$visibility == Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE) {
            //Mage::log('Url rewrite stopped. Id #' . $product->getId());
            return $this;
        }
