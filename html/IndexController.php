...
class Biztech_Mobileassistant_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function getLogoAndCurrencyAction() {
        ...
        $product = Mage::getModel('catalog/product')->getCollection()->addStoreFilter($storeId)->addAttributeToSelect('*')->addAttributeToFilter('status', array('eq' => 1));
        $product->getSelect()->limit(1);
        foreach ($product as $product_data) {
            $product = Mage::getModel('catalog/product')->load($product_data->getId());
